...
public class GLSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    @Override public boolean onTouchEvent(MotionEvent e) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            float angleH = Scene.Instance().GetAngleHorizontal();
            float angleV = Scene.Instance().GetAngleVertical();
            if(Math.abs(dx)>Math.abs(dy))
            	angleH = angleH + dx/1;
            else
            	angleV = angleV + dy/1;
            Scene.Instance().SetAngleHorizontal(angleH);
            Scene.Instance().SetAngleVertical(angleV);
        }
        mPreviousX = x;
        mPreviousY = y;
        return true;
